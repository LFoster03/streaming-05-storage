"""src/streaming/kafka_admin_foster.py - Kafka topic management example.
...
Terminal commands to run from the root project folder:
  ...
  Create the topic (safe to run repeatedly):
    uv run python -m streaming.kafka_admin_foster

  Delete the topic and recreate it (start fresh):
    uv run python -m streaming.kafka_admin_foster --recreate

  Delete the topic only:
    uv run python -m streaming.kafka_admin_foster --delete

OBS:
  Don't edit this file - it should remain a working example.
  Copy it, rename it kafka_admin_foster.py, and modify your copy.
